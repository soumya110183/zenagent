  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Agent Selection Tabs with Grouping */}
        <Tabs value={selectedAgent} onValueChange={setSelectedAgent} className="w-full">
          {/* Core Agents Section */}
          <div className="mb-4">
            <div className="text-center mb-3">
              <h3 className="text-lg font-semibold text-gray-800 bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
                🏢 Core Enterprise Agents
              </h3>
              <p className="text-sm text-gray-600">Primary analysis and validation agents</p>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6 p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg border-2 border-blue-200">
              {agents.slice(0, 4).map((agent) => (
                <TabsTrigger 
                  key={agent.id} 
                  value={agent.id}
                  className="flex flex-col items-center gap-2 p-4 data-[state=active]:bg-blue-600 data-[state=active]:text-white bg-white hover:bg-blue-50 border border-blue-100 rounded-lg shadow-sm transition-all"
                >
                  <agent.icon className="w-6 h-6" />
                  <span className="text-xs font-medium text-center leading-tight">{agent.name.replace(' Agent', '')}</span>
                </TabsTrigger>
              ))}
            </div>
          </div>
          ...
          <div className="mb-6">
            <div className="text-center mb-3">
              ...
              <p className="text-sm text-gray-600">Advanced vector, knowledge, and conversion agents</p>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border-2 border-green-200">
              {agents.slice(4).map((agent) => (
                <TabsTrigger 
                  key={agent.id} 
                  value={agent.id}
                  className="flex flex-col items-center gap-2 p-4 data-[state=active]:bg-green-600 data-[state=active]:text-white bg-white hover:bg-green-50 border border-green-100 rounded-lg shadow-sm transition-all"
                >
                  <agent.icon className="w-6 h-6" />
                  <span className="text-xs font-medium text-center leading-tight">{agent.name.replace(' Agent', '')}</span>
                </TabsTrigger>
